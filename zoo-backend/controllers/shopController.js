...
export const getAllShopItems = async (req, res) => {
  try {
    const [items] = await db.query(`
      SELECT * FROM Item 
      WHERE Item_ID != 9000 AND is_active = TRUE
      ORDER BY Item_Name
    `);
    res.json(items);
  } catch (error) {
...
export const getShopItemById = async (req, res) => {
  try {
    const { id } = req.params;
    const [items] = await db.query(
      "SELECT * FROM Item WHERE Item_ID = ? AND is_active = TRUE",
      [id]
    );
...
export const addShopItem = async (req, res) => {
  try {
    ...
    const [newItem] = await db.query("SELECT * FROM Item WHERE Item_ID = ?", [
      result.insertId,
    ]);
...
export const deleteShopItem = async (req, res) => {
  try {
    ...
    // This preserves purchase history and transaction data
    await db.query("UPDATE Item SET is_active = FALSE WHERE Item_ID = ?", [id]);
